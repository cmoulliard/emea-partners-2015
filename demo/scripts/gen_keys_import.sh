...
cd gerrit-admin-ssh/
ssh-keygen -b 4096 -t rsa -f id_rsa -q -N "" -C "admin@fabric8.io"
cd ..

echo ">> Genenate new keys for jenkins & sonar users"
mkdir gerrit-users-ssh-keys
cd gerrit-users-ssh-keys
ssh-keygen -b 4096 -t rsa -f id_jenkins-rsa -q -N "" -C "jenkins@fabric8.io"
ssh-keygen -b 4096 -t rsa -f id_sonar-rsa -q -N "" -C "sonar@fabric8.io"
cd ..
